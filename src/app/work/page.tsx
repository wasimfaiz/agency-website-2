        ...
        <section ref={containerRef} className="min-h-screen py-20 flex items-center relative z-10">
            <div className="container mx-auto px-6 grid md:grid-cols-2 gap-20 items-center">
                ...
                <motion.div style={{ y: yText }} className="space-y-10 order-2 md:order-1 relative z-20">
                    <span className="font-mono text-sm uppercase tracking-[0.3em] text-white/40 block">
                        Chapter {project.chapter}
                    </span>
                    
                    <h2 className="text-6xl md:text-8xl font-heading font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-white/40 leading-tight">
                        {project.title}
                    </h2>
                    
                    <h3 className="text-2xl md:text-3xl text-white font-light italic border-l-2 border-white/20 pl-6">
                        "{project.tagline}"
                    </h3>
                    
                    <div className="space-y-6 text-lg text-white/70 leading-loose max-w-md">
                        <p>{project.description}</p>
                        <p className="text-white/50 text-base">{project.narrative}</p>
                    </div>

                    <div className="flex flex-wrap gap-3 pt-6">
                        {project.tags.map((tag, idx) => (
                            <span key={idx} className="px-4 py-2 rounded-full border border-white/10 text-xs uppercase tracking-widest text-white/40 hover:bg-white hover:text-black transition-colors cursor-default">
                                {tag}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="pt-10">
                         <div className="group flex items-center gap-4 cursor-pointer">
                             <div className="h-[1px] w-20 bg-white group-hover:w-32 transition-all duration-500" />
                             <span className="uppercase tracking-[0.2em] text-sm">Read Case Study</span>
                         </div>
                    </div>
                </motion.div>
                ...
                <div className="order-1 md:order-2 relative aspect-[3/4] md:aspect-[4/5] overflow-hidden rounded-lg shadow-2xl">
                    <motion.div style={{ scale: scaleImg }} className="absolute inset-0 bg-[#111]">
                         {/* Placeholder Image Div */}
                         <div 
                           className="w-full h-full opacity-60" 
                           style={{ 
                             background: `radial-gradient(circle at center, ${project.color}, #000 80%)`
                           }} 
                         />
                    ...
                    <motion.div style={{ opacity: opacityOverlay }} className="absolute inset-0 bg-black pointer-events-none" />
                    
                    {/* Big Letter Watermark */}
                    <div className="absolute bottom-[-10%] right-[-10%] text-[20rem] font-bold text-white/5 font-heading pointer-events-none select-none leading-none">
                        {project.chapter}
                    </div>
                </div>
            </div>
        ...
        <section ref={targetRef} className="relative h-[300vh] bg-[#050505]">
            <div className="sticky top-0 flex h-screen items-center overflow-hidden">
                <div className="absolute top-20 left-6 z-20 md:left-20">
                    <h2 className="text-sm font-mono uppercase tracking-[0.4em] text-white/40 mb-2">R&D Department</h2>
                    <h3 className="text-5xl font-heading font-bold">Concept Lab</h3>
                </div>

                <motion.div style={{ x }} className="flex gap-20 pl-[50vw] pr-20">
                    {concepts.map((concept, i) => (
                        <div key={i} className="w-[80vw] md:w-[60vw] h-[60vh] md:h-[70vh] shrink-0 bg-[#0a0a0a] border border-white/5 rounded-3xl relative overflow-hidden group flex flex-col justify-end p-10 md:p-20 hover:border-white/20 transition-colors">
                             <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/80" />
                             <div className="absolute top-10 right-10 opacity-20 group-hover:opacity-100 transition-opacity">
                                 <span className="text-xs font-mono border border-white rounded-full px-3 py-1">Experiment 00{i+1}</span>
                             </div>
                             
                             <div className="relative z-10 space-y-4">
                                 <h4 className="text-4xl md:text-6xl font-heading font-bold">{concept.title}</h4>
                                 <p className="text-xl text-white/60 max-w-lg">{concept.desc}</p>
                             </div>
                             
                             <div className="absolute inset-0 -z-10 opacity-30 group-hover:scale-110 transition-transform duration-[1.5s]">
                                  {/* Abstract Visual Placeholder */}
                                  <div className={`w-full h-full bg-gradient-to-tr from-gray-900 via-gray-800 to-black`} /> 
                             </div>
                        </div>
                    ))}
                    ...
                    <div className="w-[50vw] md:w-[30vw] h-[60vh] md:h-[70vh] shrink-0 flex items-center justify-center">
                         <div className="text-center space-y-6">
                             <h4 className="text-4xl font-heading font-bold">What's Next?</h4>
                             <Link href="/#contact" className="inline-block border-b border-white pb-1 text-xl hover:opacity-50 transition-opacity">
...
export default function WorkPage() {
  ...
  return (
    <div ref={containerRef} className="bg-[#020202] text-[#ededed] selection:bg-white/20 font-sans min-h-screen relative overflow-clip">
      <Navbar />
      ...
      <section className="min-h-[100dvh] flex flex-col items-center justify-center relative px-6 pt-40">
          <div className="text-center space-y-10 max-w-5xl z-10 relative">
              <motion.p 
                ...
                className="font-mono text-sm uppercase tracking-[0.5em] text-white/40"
              >
                  Our Philosophy
              </motion.p>
              ...
              >
                  ...
                  <span className="text-white/30 italic font-serif">We craft legacies.</span>
              </motion.h1>
              ...
              >
                  ...
                  <span className="block mt-10 text-xs uppercase tracking-widest opacity-50 animate-bounce">Scroll to Begin</span>
              </motion.p>
          </div>
          
          {/* Background Ambient Glow */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
              <div className="absolute top-[-20%] left-[20%] w-[60vw] h-[60vw] bg-purple-900/10 blur-[150px] rounded-full" />
              <div className="absolute bottom-[-20%] right-[20%] w-[60vw] h-[60vw] bg-blue-900/10 blur-[150px] rounded-full" />
          </div>
      </section>
      ...
      <div className="relative">
          {/* Connecting Line */}
          <div className="absolute left-[20px] md:left-1/2 top-0 bottom-0 w-[1px] bg-white/5 -translate-x-1/2 z-0 hidden md:block" />
      ...
      {/* --- FOOTER CTA --- */}
      <section className="h-[80vh] flex flex-col items-center justify-center bg-[#020202] relative">
          <div className="text-center z-10 space-y-8">
              <h2 className="text-[clamp(3rem,8vw,7rem)] font-heading font-bold leading-none">
                  Your Story <br/> Starts Here.
              </h2>
              <Link href="/#contact" className="inline-block bg-white text-black px-12 py-5 rounded-full font-bold text-xl hover:scale-105 transition-transform">
                  Begin The Journey
              </Link>
          </div>
           {/* Grid Pattern Overlay */}
